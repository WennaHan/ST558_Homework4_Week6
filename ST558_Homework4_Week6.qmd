...
# change the first column to list column
article_info <- select(article_info,-source)
article_info
```
...
# Query for the topic "Business" from the past 30 days
result_business <- query_news_api(query = "Business", from_date = Sys.Date() - 30, api_key = api_key)
print(result_business)
```

```{r}
# Query for the topic "Tesla" from 2024-04-30
result_tesla <- query_news_api(query = "Tesla", from_date = as.Date("2024-04-30"), api_key = api_key)
print(result_tesla)
```
```{r}
# Query for the topic "Tesla" from 2024-04-30
result_fashion <- query_news_api(query = "fashion", from_date = 2024-04-30, api_key = api_key)
print(result_fashion)
```
```{r}
result_fashion$source
```
